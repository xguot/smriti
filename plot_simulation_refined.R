...
results <- data.frame(
  ...
  Condition = c(rep("Normal (N=1000)", 3), rep("Normal (N=200)", 3), rep("Lognormal (N=200)", 3)),
  Bias = c(0.005, 0.080, 0.006,  # Normal 1000
           0.157, 0.117, 0.012,  # Normal 200
           NA,   -0.182, -0.180)  # Lognormal 200 (FIML NA)
)

results$Condition <- factor(results$Condition, levels = c("Normal (N=1000)", "Normal (N=200)", "Lognormal (N=200)"))
results$Method <- factor(results$Method, levels = c("smriti", "missForest", "FIML"))

# Define colors
method_colors <- c("smriti" = "#009E73", "missForest" = "#56B4E9", "FIML" = "#E69F00")

p <- ggplot(results, aes(x = Bias, y = Method, color = Method)) +
  # Reference line at zero
  geom_vline(xintercept = 0, linetype = "dashed", color = "gray50", linewidth = 0.8) +
  # Lollipop lines
  geom_segment(aes(x = 0, xend = Bias, y = Method, yend = Method), linewidth = 1.2, alpha = 0.6) +
  # Points
  ...
  labs(
    ...
    subtitle = "SMRITI Anchoring: Consistent minimal variance bias in Normal tests, highly stable with Lognormal.",
    x = "Relative Bias of Covariance Trace",
    y = ""
  )

ggsave("smriti_simulation_refined.pdf", plot = p, width = 8, height = 7, device = "pdf")
ggsave("smriti_simulation_refined.png", plot = p, width = 8, height = 7, dpi = 300)
cat("Successfully generated smriti_simulation_refined.pdf and smriti_simulation_refined.png\n")
